...
train_loss =[]
for cv in [1,2,3,4,5]:
    temp = scio.loadmat('/home/PJLAB/liumianxin/Desktop/VIEW_codes/kfold/'+pet_type+'_sub/shuffled_index_cv' + str(cv))
    ...
    while not qualified:
        ...
        for epoch in range(EPOCH):
            for sub,(b_x, b_y) in enumerate(train_loader):
                ...
                if epoch>=0 and tr_accuracy>=0.7:
                    predicted_all = []
                    predict_p = []
                    test_y_all = []
                    model.eval()
                    ...
                    if auc >= auc_baseline and sens > 0.50 and spec > 0.50:
                        auc_baseline = auc
                        torch.save(model.state_dict(), '/home/PJLAB/liumianxin/Desktop/VIEW_codes/models_sub/'+ 'model_cv' + str(cv) + '_'+pet_type+'_test.pth')
