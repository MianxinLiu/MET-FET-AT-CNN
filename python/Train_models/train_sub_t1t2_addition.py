...
data = torch.load('/home/PJLAB/liumianxin/Desktop/VIEW_codes/datasets/met_slice_crop_t1t2_v2.pth')
...
for cv in [1,2,3,4,5]:
    ...
    while not qualified:
        lr = 0.001
        auc_baseline = 0.60
        dataset_train = TensorDataset(torch.cat((x[train_idx, :, :, :], x2),dim=0), torch.cat((y[train_idx],y2),dim=0))
        ...
        for epoch in range(EPOCH):
            if epoch>=8 and input == 'fet':  # for t1 t2 we use data augmented training throughout the process
            ...
            for sub,(b_x, b_y) in enumerate(train_loader):
                ...
                if epoch>=8 and tr_accuracy>=0.70:
                    predicted_all = []
                    predict_p = []
                    test_y_all = []
                    model.eval()
                    ...
                    if auc >= auc_baseline and sens > 0.50 and spec >0.50:
                        auc_baseline = auc
                        torch.save(model.state_dict(), '/home/PJLAB/liumianxin/Desktop/VIEW_codes/models_t1t2_v2/'+ pet_type+'_model_cv' + str(cv) + '_'+input+'_addition.pth')
